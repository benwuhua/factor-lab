...
class AutoresearchCliTests(unittest.TestCase):
    ...
    def test_run_multilane_autoresearch_script_exposes_main(self):
        root = Path(__file__).resolve().parents[1]
        script = root / "scripts" / "autoresearch" / "run_multilane_autoresearch.py"

        spec = importlib.util.spec_from_file_location("run_multilane_autoresearch", script)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
