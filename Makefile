...
SIGNAL_COMBO_SPEC ?= $(COMBO_SPEC)
SIGNAL_COMBO_SPEC_ARG = $(if $(SIGNAL_COMBO_SPEC),--combo-spec $(SIGNAL_COMBO_SPEC),)
...
DAILY_DATA_FETCH_DIVIDENDS ?= 0
DAILY_DATA_FETCH_DIVIDENDS_ARG = $(if $(filter 1 true yes TRUE YES,$(DAILY_DATA_FETCH_DIVIDENDS)),--fetch-dividends,)
DAILY_DATA_DIVIDEND_PROVIDER ?= tushare
DAILY_DATA_DIVIDEND_PROVIDER_ARG = $(if $(DAILY_DATA_DIVIDEND_PROVIDER),--dividend-provider $(DAILY_DATA_DIVIDEND_PROVIDER),)
DAILY_DATA_FETCH_DISCLOSURE_EVENTS ?= 0
DAILY_DATA_FETCH_DISCLOSURE_EVENTS_ARG = $(if $(filter 1 true yes TRUE YES,$(DAILY_DATA_FETCH_DISCLOSURE_EVENTS)),--fetch-disclosure-events,)
...
DAILY_DATA_TUSHARE_SECURITY_MASTER_HISTORY ?= 1
DAILY_DATA_TUSHARE_SECURITY_MASTER_HISTORY_ARG = $(if $(filter 1 true yes TRUE YES,$(DAILY_DATA_TUSHARE_SECURITY_MASTER_HISTORY)),--tushare-security-master-history,)
DAILY_DATA_TUSHARE_SECURITY_MASTER_START_DATE ?= 2015-01-01
DAILY_DATA_TUSHARE_SECURITY_MASTER_START_DATE_ARG = $(if $(DAILY_DATA_TUSHARE_SECURITY_MASTER_START_DATE),--tushare-security-master-start-date $(DAILY_DATA_TUSHARE_SECURITY_MASTER_START_DATE),)
DAILY_DATA_TUSHARE_SECURITY_MASTER_OUTPUT ?= data/vendor/security_master_history_tushare.csv
DAILY_DATA_TUSHARE_SECURITY_MASTER_OUTPUT_ARG = $(if $(DAILY_DATA_TUSHARE_SECURITY_MASTER_OUTPUT),--tushare-security-master-output $(DAILY_DATA_TUSHARE_SECURITY_MASTER_OUTPUT),)
...
DAILY_DATA_RESEARCH_CONTEXT_LIMIT ?=
DAILY_DATA_RESEARCH_CONTEXT_LIMIT_ARG = $(if $(DAILY_DATA_RESEARCH_CONTEXT_LIMIT),--research-context-limit $(DAILY_DATA_RESEARCH_CONTEXT_LIMIT),)
DAILY_DATA_OFFSET ?= 0
...
research-data-domains:
	$(PYTHON) scripts/build_research_data_domains.py \
		...
		$(DAILY_DATA_FETCH_DIVIDENDS_ARG) \
		$(DAILY_DATA_DIVIDEND_PROVIDER_ARG) \
		$(DAILY_DATA_FETCH_DISCLOSURE_EVENTS_ARG) \
...
daily-data-update:
	$(PYTHON) scripts/update_daily_data.py \
		...
		$(DAILY_DATA_TUSHARE_SECURITY_MASTER_HISTORY_ARG) \
		$(DAILY_DATA_TUSHARE_SECURITY_MASTER_START_DATE_ARG) \
		$(DAILY_DATA_TUSHARE_SECURITY_MASTER_OUTPUT_ARG) \
		$(DAILY_DATA_RQDATA_SECURITY_MASTER_HISTORY_ARG) \
		$(DAILY_DATA_RQDATA_START_DATE_ARG) \
		$(DAILY_DATA_RQDATA_OUTPUT_ARG) \
		$(DAILY_DATA_LIMIT_ARG) \
		$(DAILY_DATA_RESEARCH_CONTEXT_LIMIT_ARG) \
		$(DAILY_DATA_OFFSET_ARG) \
